...
class Cafe:
    ...
    def take_order(self, customer):
        ...
        self.kitchen['pending_orders'].append({'table': customer['table'], 'order': order, 'preparation_time': random.randint(5, 10)})
    ...
    def process_pending_orders(self):
        for order_info in self.kitchen['pending_orders']:
            order_info['preparation_time'] -= 1
            if order_info['preparation_time'] <= 0:
                self.kitchen['pending_orders'].remove(order_info)
                self.serve_dish(order_info['table'], order_info['order'])
